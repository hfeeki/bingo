...
func (self *TrieRouter) AddRoutes(routes ...Route) error {
...
func (self *TrieRouter) Start() error {
	...
	for i, _ := range self.Routes {
		...
		self.index[route] = i
		// insert in the Trie
		err := self.trie.AddRoute(pattern, route)
		if err != nil {
			return err
		}
	}
...
func (self *TrieRouter) FindRouteFromURL(method string, url_obj *url.URL) (*Route, map[string]string) {

	// lookup the routes in the Trie
	// TODO verify url encoding
	matches := self.trie.FindRoutes(method + url_obj.Path)
...
func (self *TrieRouter) FindRoute(method, url_str string) (*Route, map[string]string, error) {
	...
	route, params := self.FindRouteFromURL(method, url_obj)
