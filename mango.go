...
func (this Env) Logger() *log.Logger {
	if this["bingo.logger"] == nil {
		this["bingo.logger"] = log.New(os.Stdout, "", log.Ldate|log.Ltime)
	}

	return this["bingo.logger"].(*log.Logger)
}

func (this Env) Request() *Request {
	return this["bingo.request"].(*Request)
}

func (this Env) Session() map[string]interface{} {
	return this["bingo.session"].(map[string]interface{})
}
	...
	if len(r) <= 1 {
		...
		return func(input Env) (Status, Headers, Body) {
			return r[0](input, func(Env) (Status, Headers, Body) {
				panic("Core Bingo App should never call it's upstream function.")
			})
		}
	}
...
func (this *Stack) HandlerFunc(app App) http.HandlerFunc {
	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		env["bingo.request"] = &Request{r, nil}
		env["bingo.version"] = Version()
...
func (this *Stack) Run(app App) error {
	...
	fmt.Println("Starting Bingo Stack On:", this.Address)
